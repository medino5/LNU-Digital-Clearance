...
class ClearanceRequestController extends Controller
{
    ...
    /**
     * GET /api/clearance/history/{id}
     *
     * Returns one clearance request with its signatures for the authenticated student.
     */
    public function showHistoryDetail(Request $request, $id)
    {
        $user = $request->user();

        if (!$user || !$user->is_student) {
            return response()->json([
                'message' => 'Only students can view clearance history details.',
            ], 403);
        }

        try {
            $clearance = ClearanceRequest::where('student_id', $user->id)
                ->where('id', $id)
                ->with(['signatures.designation'])
                ->first();
            ...
            if (!$clearance) {
                return response()->json([
                    'message' => 'Clearance history record not found.',
                ], 404);
            }

            $signatures = $clearance->signatures->map(function ($signature) {
                return [
                    'id' => $signature->id,
                    'status' => $signature->status,
                    'rejection_reason' => $signature->rejection_reason,
                    ...
                    'approved_at' => $signature->approved_at,
                    'designation' => [
                        'id' => $signature->designation?->id,
                        'name' => $signature->designation?->name,
                    ],
                ];
            })->values();
            ...
            Log::error('Failed to fetch clearance history detail', [
                'user_id' => $user->id,
            ...
            return response()->json([
                'message' => 'Failed to fetch clearance history detail.',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
